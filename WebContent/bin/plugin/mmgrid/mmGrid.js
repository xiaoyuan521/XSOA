...
!function($){
    var MMGrid = function (element, options) {
        ...
        this._initEvents();
        this._setColsWidth();
        this.allRowCount = 0;  //ljg 2013-07-10
    ...
    MMGrid.prototype = {
        ...
        _initLayout: function($el){
            ...
            var btnBackboard ="" ;
           //2013/08/07修正 by 武江波 start
            if (opts.btnBackboard || opts.btnBackboard == null) {
            	btnBackboard = '<a class="mmg-btnBackboardDn"></a>';
			}
          //2013/08/07修正 by 武江波 end
            var mmGrid = [
                '<div class="mmGrid">',
                    '<style></style>',
            ...
            if(opts.checkCol){
                ...
                opts.cols.unshift({title:chkHtml,width: 40, align: 'center' ,lockWidth: true, checkCol: true, renderer:function(){
                    return '<input type="checkbox" class="mmg-check">';
                }});
            }

            if(opts.indexCol){
                opts.cols.unshift({title:'No',width: opts.indexColWidth, align: 'center' ,lockWidth: true, indexCol:true, renderer:function(val,item,rowIndex){
        ...
        , _initEvents: function(){
            ...
            $head.on('click', '.mmg-title', function(){
                ...
                //2013-07-29 ljg 判断无数据时不排序 Start
                if(!that.rowsLength() || (that.rowsLength() === 1 && $body.find('tr.emptyRow').length === 1)){
                    return;
                }
                //2013-07-29 ljg 判断无数据时不排序 end
            ...
            //ljg 2013-07-05 Start
            //下拉列表select改变事件
            $body.on('change','.select',function(e){
                var $this = $(this);
                var rowIndex = $this.parent().parent().parent().index(); //当前行
                var colIndex = $this.parent().parent().index();          //当前列
                var $tbody = $body.find('tbody');
                var $tr = $tbody.find('tr').eq(rowIndex); //取得当前行对象
                var currItem = $tr.data('item'); //取得当前行item
                currItem[opts.cols[colIndex].name]=e.target.value; //设置当前行item
                $tr.data('item', currItem); //更新当前行数据
            ...
            $body.on('change','.checkbox',function(e){
                ...
                var $tr = $tbody.find('tr').eq(rowIndex); //取得当前行对象
                var currItem = $tr.data('item'); //取得当前行item
                currItem[opts.cols[colIndex].name]=newValue;//e.target.value; //设置当前行item
                $tr.data('item', currItem); //更新当前行数据
            ...
            if(opts.checkCol){
                $head.find('th .checkAll').on('click', function(){
                    ...
                    //增加全选按钮的响应事件 2013-08-01 ljg Start
                    $body.triggerHandler('checkAll');
                    //增加全选按钮的响应事件 2013-08-01 ljg End
                });
            }
        ...
        , _loadAjax: function(args){
            ...
            $.ajax({
                ...
                //ljg 2013/07/02 start
                //全选后,到下一页也要选中所有
                /*var $head = that.$head;
                if($head.find('th .checkAll').prop('checked')){
                	that.select('all');
                }*/
                //ljg 2013/07/02 end
        ...
        //清除表格数据 2013-08-01 Start
        , clear: function(){
        	this._insertEmptyRow();
        	var plugin = this.opts.plugins[0];
        	if(typeof plugin == "undefined"){//未加分页器
        		
        	}else{//取得分页器
        		plugin.clear();
        	}
        }
        //清除表格数据 2013-08-01 End
        , load: function(args){
        	//ljg 2013-07-13 Start
        	//解决:查询后选第二页,然后再查询,序号不正确的问题.
        	/*if((typeof args) != "undefined"){ //保存有效的查询条件//保存条件方法已改
        		this._saveArgs = args;
        	}*/
        	if(this._loadCount>0){//第一次调用load,则不处理
            	if(typeof args == "undefined"){//分页器调用load(如上一页,下一页)
                    //读取上一次的查询条件
            		//args = this._saveArgs;
            	}else{//程序中指定条件调用
            		//设置分页器当前页为首页
            ...
            //ljg 2013-08-20 Start 保存有效的查询条件
        	if((typeof args) != "undefined"){ //保存有效的查询条件
                //opt的params可以使函数，例如收集过滤的参数
                if($.isFunction(opts.params)){
                	opts.params = $.extend(opts.params(), args);
                }else if($.isPlainObject(opts.params)){
                	opts.params = $.extend(opts.params, args);
                }
        	}
        ...
        , resize: function(){
          ...
          //2013-07-02 ljg start
          //对外公开方法:判断全选是否被选中
        }, checkAllStatus:function(){
        	var $head = this.$head;
            return $head.find('th .checkAll').prop('checked');
        	//2013-07-02 ljg end
        }
        ...
        //ljg 2013-07/11 Start
        //显示表格的总行数
        , rowsAllLength: function(){
