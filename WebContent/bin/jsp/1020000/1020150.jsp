		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				var cols = [
					...
					{ title:'开始时间', name:'CCSQ_KSSJ' ,width:150, sortable:true, align:'center',lockDisplay: true  },
					{ title:'结束时间', name:'CCSQ_JSSJ' ,width:150, sortable:true, align:'center',lockDisplay: true  },
					{ title:'出差天数', name:'CCSQ_CCTS' ,width:80, sortable:true, align:'center',lockDisplay: true  },
					{ title:'状态', name:'SPZT' ,width:80, sortable:true, align:'center',lockDisplay: true  },
				...
				//初始化表格
	   			mmg = $('.mmg').mmGrid({
					...
					method: 'post',
					params:{CMD : "<%=Servlet1020150.CMD_SELECT%>"},
					remoteSort:true,
					sortName: 'CCSQ_SQID',
					sortStatus: 'ASC',
					root: 'items',
				...
				$('#btnDel').on('click', function() {
					...
					layer.confirm('是否删除出差申请？', function() {
						layer.close(layer.index);
						if (deleteData(arrList[0].CCSQ_SQID) == true) {
							//重新查询数据
							loadGridByBean();
							mmg.deselect('all');
						}
			...
			//定义查询bean
			function makeBeanIn(strCCSQ_SQID, strCCSQ_KSSJ, strCCSQ_JSSJ, strCCSQ_SPZT) {
			...
			function btn_Add() {
				obj = new Object();
				obj.optionFlag = "Add";
				iframeLayerOpen("<%=basePath%>/bin/jsp/1020000/1020151.jsp", "700px", "500px");
			}
			//修改按钮点击方法
			function btn_Upd() {
				obj = new Object();
	    		...
	    		if (arrList[0].CCSQ_SPZT != '0' && arrList[0].CCSQ_SPZT != '4' && arrList[0].CCSQ_SFWJ == '0') {
	    			layer.alert('此申请单处于审批环节，无法进行修改！', 0, '友情提示');
	    			return;
	    		}
	    		if (arrList[0].CCSQ_SFWJ == '1') {
				...
				$.ajax({
					...
					success : function(response) {
						...
						if (strResult == "SUCCESS") {
							layer.msg('恭喜：删除出差申请信息成功！', 1, 9);
							blnRet = true;
						} else if (strResult == "FAILURE") {
							layer.msg('对不起：删除出差申请信息失败！', 1, 8);
							blnRet = false;
						} else if (strResult == "EXCEPTION") {
							layer.msg('友情提示：删除出差申请信息出错！', 1, 0);
							blnRet = false;
						}
					}
				});
